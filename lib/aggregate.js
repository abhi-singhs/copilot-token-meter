...
function aggregate(events, telemetry) {
  ...
  // attribute their cost to the parent tool (typically "task").
  ...
  // summary, and so the user can tell which fraction of the spend was
  // delegated work.
  const subAgentTotals = {
    inputTokens: 0, outputTokens: 0,
    cacheReadTokens: 0, cacheWriteTokens: 0,
    reasoningTokens: 0, cost: 0, durationMs: 0, apiCalls: 0,
    usdCost: 0,
  ...
  if (haveTelemetry) {
    for (const r of telemetry.usage) {
      ...
      if (toolBatch && toolBatch.length) {
        ...
        // Sub-agent API call — attribute its cost to the parent tool (e.g.
        // `task`) via the parent_tool_call_id linkage. This keeps sub-agent
        // spend visible in the per-tool table without needing to walk the
        // sub-agent's own events.jsonl file.
  ...
  // Sub-agent USD totals — sum across the per-model split.
  for (const [m, b] of Object.entries(subAgentTotals.perModel)) {
    const usd = costForBucket(m, b);
    b.usdCost = usd == null ? 0 : usd;
    subAgentTotals.usdCost += b.usdCost;
  }
