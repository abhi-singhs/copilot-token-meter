...
const { contextWindow } = require("./pricing");
...
function bucketInit() {
  return {
    ...
    reasoningTokens: 0,
    apiCalls: 0, durationMs: 0,
    efforts: Object.create(null),
    latestQuota: null,
  };
}

// Pick the dominant reasoning_effort tier seen for a model bucket. Returns
// e.g. "high" when all observed calls used the same effort, "high+" when
// "high" was the modal choice but other tiers also appeared, or null when
// no effort field was reported (Anthropic models, older CLI builds, etc).
function dominantEffort(efforts) {
  const entries = Object.entries(efforts || {}).filter(([k, v]) => k && v > 0);
  if (entries.length === 0) return null;
  entries.sort((a, b) => b[1] - a[1]);
  const [top] = entries;
  return entries.length === 1 ? top[0] : top[0] + "+";
}
...
function aggregate(events, telemetry) {
  ...
  // toolCallId → tool name. Lets sub-agent API calls (which carry the
  // parent's `parent_tool_call_id` but no provider_call_id we can use)
  // attribute their tokens to the parent tool (typically "task").
  const toolCallIdToName = Object.create(null);
  let pendingToolBatch = null;
  // Sub-agent tracking — totals split out for the by-initiator section in
  // summary, and so the user can tell which fraction of the activity was
  // delegated work.
  const subAgentTotals = {
    inputTokens: 0, outputTokens: 0,
    cacheReadTokens: 0, cacheWriteTokens: 0,
    reasoningTokens: 0, durationMs: 0, apiCalls: 0,
    perModel: Object.create(null),
  };
  ...
  for (const ev of events) {
    ...
    switch (t) {
      ...
      case "tool.execution_start": {
        ...
        const callId = d.toolCallId || d.callId || d.id || null;
        if (callId) toolCallIdToName[callId] = name;
        if (tt) {
          tt.toolCalls++;
          tt.tools.push({ name, callId });
        }
        break;
      }
  ...
  if (haveTelemetry) {
    for (const r of telemetry.usage) {
      ...
      if (r.reasoning_effort) {
        bucket.efforts[r.reasoning_effort] = (bucket.efforts[r.reasoning_effort] || 0) + 1;
      }
      ...
      } else if (r.initiator === "sub-agent" && r.parent_tool_call_id) {
        // Sub-agent API call — attribute its tokens to the parent tool
        // (e.g. `task`) via the parent_tool_call_id linkage. This keeps
        // sub-agent activity visible in the per-tool table without needing
        // to walk the sub-agent's own events.jsonl file.
        const parentTool = toolCallIdToName[r.parent_tool_call_id] || "task";
        const tool = ensureTool(parentTool);
        if (tool) {
          tool.inputTokens      += inTok;
          tool.outputTokens     += outTok;
          tool.cacheReadTokens  += crTok;
          tool.cacheWriteTokens += cwTok;
          tool.models[m] = (tool.models[m] || 0) + 1;
        }
      }

      // Sub-agent totals broken out for the by-initiator section.
      if (r.initiator === "sub-agent") {
        subAgentTotals.inputTokens      += inTok;
        subAgentTotals.outputTokens     += outTok;
        subAgentTotals.cacheReadTokens  += crTok;
        subAgentTotals.cacheWriteTokens += cwTok;
        subAgentTotals.reasoningTokens  += reasTok;
        subAgentTotals.durationMs       += durMs;
        subAgentTotals.apiCalls         += 1;
        const sb = subAgentTotals.perModel[m] || (subAgentTotals.perModel[m] = bucketInit());
        sb.inputTokens      += inTok;
        sb.outputTokens     += outTok;
        sb.cacheReadTokens  += crTok;
        sb.cacheWriteTokens += cwTok;
        sb.reasoningTokens  += reasTok;
        sb.apiCalls         += 1;
        sb.durationMs       += durMs;
      }
    }
  ...
  // Derived totals (wire + billed, in tokens).
  ...
  // Derive a single dominantEffort label per model bucket so the renderer
  // doesn't need to know the modal-choice tie-break logic.
  for (const bucket of Object.values(perModel)) {
    bucket.dominantEffort = dominantEffort(bucket.efforts);
  }
...
module.exports = { aggregate, bucketInit, dominantEffort };
